...
def profile():
    ...
    tg_verify = database.check_telegram_verify(session.get("user_id"))
...
@limiter.limit("2 per second")
@app.route("/profile/telegram/token", methods=["GET", "POST"])
def telegram_token():
    if "user_id" not in session:
        return redirect(url_for("login"))
    ...
    if request.method == "POST":
        ...
        token_status = database.tg_token_create(user_id)
        if token_status.get("status"):
            return render_template("tg-token.html", connection_token=token_status.get("token"))
        else:
            return render_template("tg-token.html", connection_token="Ошибка во время получения токена! Обратитесь в поддержку!")

    elif request.method == "GET":
        user_id = session.get("user_id")
        token = database.get_tg_token(user_id)
        if token:
            return render_template("tg-token.html", connection_token=token)
        else:
            return render_template("tg-token.html", connection_token="Ошибка во время получения токена! Обратитесь в поддержку!")

@limiter.limit("2 per second")
@app.route("/profile/telegram/unlink-telegram")
def unlink_telegram():
    if not "user_id" in session:
        return redirect(url_for("login"))
    
    user_id = session.get("user_id")
    result = database.unlink_tg_user(user_id)
    if result.get("status"):
        return redirect(url_for("profile"))
    else:
        flash(result.get("exception"), "error")
        return redirect(url_for("profile"))
